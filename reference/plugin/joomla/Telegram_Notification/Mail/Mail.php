	...
	public function Send()
	{
		if (Factory::getConfig()->get('mailonline', 1))
		{
			...
			try
			{
                ...
                $query->from($db->quoteName('#__telegrambot_settings'));
        
                $db->setQuery($query);
                $result_db = $db->loadAssoc();
                
                $data_prepare =  '';
                $is_send = true;
                
                if (preg_match('/<meta[^>]+charset=utf-8[^>]*>/i', $this->Body, $match)) {
                   
                    $is_send = false;
                    $data_prepare = "Заказ virtuemart";
                    
                } else {
                   $data_prepare =  $this->Body;
                }
                
                               
				$result = parent::send();
				$req = array(
					'password' => $result_db['password'],
					'data' =>  $data_prepare.$result_db['email']
				);
				$senddata = json_encode($req);
                
                
                    
			if($is_send) self::sendDataToServer($senddata,$result_db['server']); 
